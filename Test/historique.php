    ...
    <body>
        ...
        <div class="row">
            ...
            <div class="large-9 push-3 columns">
                ...
                $reqcom = $bdd->prepare('SELECT Commande.idCommande, nom, prix, dateCommande, Statut FROM commande, composer, produit WHERE commande.idCommande = composer.idCommande AND composer.refProduit = produit.refProduit AND Statut="0" AND idClient ="' . $ligne1['idClient'].'"');
                $reqcom->execute() or die(print_r($reqcom->errorInfo()));
                $ligne2 = $reqcom->fetch(PDO::FETCH_ASSOC);
                $taille = $reqcom->rowCount();
                ...
                if ($ligne2 != "") {
                    ...
                    do {
                        ...
                        $nomprod = $nomprod.'<br>'.$ligne2['nom'];
                        $prixtot = $prixtot.'<br>'.$ligne2['prix'];
                        $numcom = $ligne2['idCommande'];                   
                    } while ($ligne2 = $reqcom->fetch(PDO::FETCH_ASSOC));
                    echo '<tr>';
                        echo '<td> '.$numcom.' </td>';
                        echo '<td><li> '.$nomprod.'</li></td>';
                        echo '<td> '.$newDate.'</td>';
                        echo '<td> '.$prixtot.'</td>';
                    echo '</tr>';
                }
                else
                    echo "<h5 style='text-align:center'>Vous n'avez pas de fait de commande précédement.</h5>";
                echo '</table>';
                ?>
            ...
            <div class="large-3 pull-9 columns">
                ...
                <p><img src="img/historique.jpg" /></p>
